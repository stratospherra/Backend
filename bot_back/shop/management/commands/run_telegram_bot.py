...
def products(message):
    ...
    for apple in products:
        bot.send_message(message.chat.id, f"product:{apple.name},price:{apple.price}")

@bot.message_handler(commands=['help'])
def help(message):
    bot.send_message(message.chat.id, f"help: /start, /product")
...
def price_handler(message):
    ...
    new_game = Product.objects.create(name=title, price=price)
